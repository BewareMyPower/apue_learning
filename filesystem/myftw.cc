...
public:
    ...
    const char* basename() const { return _basestr; }
...
public:
    ...
    FileTreeIterator(const char* dirname, CallBack func) : _path(dirname), _func(func) {}
...
PathBuffer::PathBuffer(const char* dirname) {
    _len = strlen(dirname);
    if (_len >= PATH_MAX)
        err_quit("%s is too long", dirname);
    strncpy(_str, dirname, _len);
    _str[_len] = '\0';
}
...
inline void PathBuffer::set_basestr() {
    for (_basestr = _str + _len; _basestr != _str && *_basestr != '/'; --_basestr) {}
    ++_basestr;
}

inline bool PathBuffer::is_dir() const {
    if (!lstat())
        return false;
